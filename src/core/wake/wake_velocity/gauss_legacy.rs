//! GaussLegacy (GCH) Wake Velocity Model
//!
...
/// GaussLegacy (GCH) wake velocity model
#[derive(Debug, Clone)]
pub struct GaussLegacyVelocityDeficit {
    pub base: BaseModel,
    pub ka: Float,
    pub kb: Float,
...
impl GaussLegacyVelocityDeficit {
    pub fn new(ka: Float, kb: Float, initial_wake_width: Float) -> Self {
        ...
        Self {
            base: BaseModel::new(params, "gauss_legacy"),
            ka,
            kb,
            initial_wake_width,
        }
...
impl Default for GaussLegacyVelocityDeficit {
    fn default() -> Self {
        Self::default()
    }
}
...
impl GaussLegacyVelocityDeficit {
    ...
    fn calculate_deficit(&self, axial_induction: Float, x: Float, r0: Float) -> Float {
        if axial_induction <= 0.0 {
            return 0.0;
        }

        let sigma = self.calculate_sigma_y(x, r0, 0.0);
...
mod tests {
    ...
    #[test]
    fn test_gauss_legacy_creation() {
        let gauss = GaussLegacyVelocityDeficit::new(0.1, 0.05, 0.5);
    ...
    #[test]
    fn test_gauss_legacy_default() {
        let gauss = GaussLegacyVelocityDeficit::default();
        assert_eq!(gauss.ka, 0.1);
        assert_eq!(gauss.kb, 0.05);
        assert_eq!(gauss.initial_wake_width, 0.5);
    }
    ...
    #[test]
    fn test_gauss_legacy_prepare_function() {
        let gauss = GaussLegacyVelocityDeficit::default();
